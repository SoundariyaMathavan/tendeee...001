...
export async function POST(request: NextRequest) {
  try {
    ...
    // Enhanced ML Analysis with new criteria
    const mlAnalysisData = {
      bidAmount: parseFloat(bidAmount),
      projectBudget: project.budget,
      ongoingProjects: body.ongoingProjects || 0,
      conflictingDeadlines: body.conflictingDeadlines || 0,
      employeeCount: body.employeeCount || 0,
      companyCapacity: body.companyCapacity || 0,
      financialStability: body.financialStability || "average",
    ...
    let mlAnalysis = null
    try {
      const mlResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/ml/analyze-bid`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
        },
        body: JSON.stringify(mlAnalysisData)
      })
    ...
    const newBid = {
      ...
      financialStability: body.financialStability || "average",
      status: "submitted",
      ...analysisResult, // Include basic AI analysis results
      mlAnalysis: mlAnalysis, // Include enhanced ML analysis
      createdAt: new Date(),
      updatedAt: new Date(),
    }
