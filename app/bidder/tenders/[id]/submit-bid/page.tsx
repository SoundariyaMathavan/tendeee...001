...
import { Building2, ArrowLeft, FileText, DollarSign, Clock, Award, X, TrendingUp } from "lucide-react"
...
export default function SubmitBidPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Validate bid amount against minimum requirement
      const bidAmount = parseFloat(formData.bidAmount)
      const minBidAmount = project.budget * 0.8
      
      if (bidAmount < minBidAmount) {
        toast({
          title: "Invalid Bid Amount",
          description: `Bid amount must be at least ${minBidAmount.toLocaleString()} (80% of project budget)`,
          variant: "destructive",
        })
        return
      }
      ...
      const analysisMessage = bidData.mlAnalysis ? 
        `Bid submitted successfully! 
        AI Score: ${bidData.aiScore}/100
        Capacity Score: ${bidData.mlAnalysis.capacityScore}/100
        Risk Assessment: ${bidData.mlAnalysis.riskLevel}
        Recommendation: ${bidData.mlAnalysis.recommendation}` :
        `Bid submitted successfully! AI Score: ${bidData.aiScore}/100`
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      required
                      min={project?.budget * 0.8} // Minimum 80% of project budget
                    />
                    {project?.budget && (
                      <p className="text-sm text-gray-500 mt-1">
                        Minimum bid: ${(project.budget * 0.8).toLocaleString()} (80% of project budget)
                      </p>
                    )}
                  </div>
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {/* Ongoing Projects */}
                      <div>
                        <Label htmlFor="ongoingProjects">Number of Ongoing Projects</Label>
                        <Input
                          id="ongoingProjects"
                          type="number"
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          Current active projects your company is handling
                        </p>
                      </div>

                      {/* Conflicting Deadlines */}
                      <div>
                        <Label htmlFor="conflictingDeadlines">Projects with Conflicting Deadlines</Label>
                        <Input
                          id="conflictingDeadlines"
                          type="number"
                          value={formData.conflictingDeadlines}
                          onChange={(e) => setFormData({ ...formData, conflictingDeadlines: e.target.value })}
                          placeholder="e.g., 1"
                          required
                        />
                        <p className="text-xs text-gray-500 mt-1">
                          Projects with deadlines overlapping this project
                        </p>
                      </div>
                      ...
                      {/* Company Capacity */}
                      <div>
                        <Label htmlFor="companyCapacity">Available Capacity (%)</Label>
                        <Input
                          id="companyCapacity"
                          type="number"
                          value={formData.companyCapacity}
                          onChange={(e) => setFormData({ ...formData, companyCapacity: e.target.value })}
                          placeholder="e.g., 70"
                          min="0"
                          max="100"
                          required
                        />
                        <p className="text-xs text-gray-500 mt-1">
                          Percentage of company capacity available for new projects
                        </p>
                      </div>

                      {/* Financial Stability */}
                      <div className="md:col-span-2">
                        <Label htmlFor="financialStability">Financial Stability Rating</Label>
                        <Select 
                          value={formData.financialStability} 
                          onValueChange={(value) => setFormData({ ...formData, financialStability: value })}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder="Select financial stability rating" />
                          </SelectTrigger>
                          <SelectContent>
                            ...
                            <SelectItem value="poor">Poor (D)</SelectItem>
                          </SelectContent>
                        </Select>
                        <p className="text-xs text-gray-500 mt-1">
                          Your company's financial stability and credit rating
                        </p>
                      </div>
                    </div>
                  </div>
